...
// buyerPickupTerminal is a BrittoTerminal enum on the backend, not free text —
// same fixed set used in rental-request.tsx's pickup point selector.
const PICKUP_POINTS = [
    ...
    try {
      ...
      if (err?.response?.data?.errorType === "VERIFICATION_ERROR") {
        Alert.alert(
          ...
          "Please verify your account before buying something.",
        );
      } else {
            ...
            <View className="mb-4">
              ...
              <View className="flex-row flex-wrap gap-2">
                {PICKUP_POINTS.map((pt) => {
                  const active = buyerPickupTerminal === pt.value;
                  return (
                    <TouchableOpacity
                      key={pt.value}
                      onPress={() => setBuyerPickupTerminal(pt.value)}
              ...
              {errors.buyerPickupTerminal && (
                <Text className="text-red-400 text-xs mt-2 ml-0.5">
                  {errors.buyerPickupTerminal}
                </Text>
              )}
            </View>
